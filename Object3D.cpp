...
//Load a object 3d using assimp
void Object3D::loadASSIMP(std::string const& path)
{
	//Creates an assimp importer
	Assimp::Importer importer;

	//Creates a assimp scene and imports the object
	const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_FlipUVs /*| aiProcess_CalcTangentSpace*/);

	//Error checkinig the assimp scene
	if (!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode)
	{
		SDL_Log("Error loading object with assimp %s", importer.GetErrorString());;
		return;
	}
	...
	for (unsigned int i = 0; i < mesh->mNumVertices; i++)
	{
		...
		//Texture coordinates of the object if present
